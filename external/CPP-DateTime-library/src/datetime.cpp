...
namespace jed_utils
{
    ...
    int datetime::days_in_month() const {
      static int month_days[] = {31, 28, 31, 30,
                                 31, 30, 31, 31,
                                 30, 31, 30, 31};
      if (get_month() == 2) {
        return month_days[get_month() - 1] + is_leapyear();
      }
      return month_days[get_month() - 1];
    }

    int datetime::days_in_year() const {
      return 365 + is_leapyear();
    }

    int datetime::days_to(const datetime& other) const {
      return std::difftime(std::mktime(timeInfo), std::mktime(other.timeInfo)) / (60 * 60 * 24);
    }
